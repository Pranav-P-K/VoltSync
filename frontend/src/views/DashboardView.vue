    ...
    <div class="px-4 py-6 sm:px-0">
      ...
      <div class="bg-white p-4 rounded-lg shadow mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <input v-model.number="filters.minPower" @input="applyFilters" type="number" placeholder="Min Power (kW)"
            class="border border-gray-300 rounded-md px-3 py-2" />

          <input v-model.number="filters.maxPower" @input="applyFilters" type="number" placeholder="Max Power (kW)"
            class="border border-gray-300 rounded-md px-3 py-2" />
        </div>
      </div>
      ...
      <div v-if="showAddForm || editingStation"
        ...
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white" @click.stop>
          ...
          <form @submit.prevent="submitForm" class="space-y-4">
            <input v-model="stationForm.name" type="text" placeholder="Station Name" required
              class="w-full border border-gray-300 rounded-md px-3 py-2" />

            <div class="grid grid-cols-2 gap-2">
              <input v-model.number="stationForm.location.latitude" type="number" step="any" placeholder="Latitude"
                required class="border border-gray-300 rounded-md px-3 py-2" />
              <input v-model.number="stationForm.location.longitude" type="number" step="any" placeholder="Longitude"
                required class="border border-gray-300 rounded-md px-3 py-2" />
            </div>

            <input v-model="stationForm.location.address" type="text" placeholder="Address (optional)"
              class="w-full border border-gray-300 rounded-md px-3 py-2" />

            <select v-model="stationForm.status" required class="w-full border border-gray-300 rounded-md px-3 py-2">
            ...
            <input v-model.number="stationForm.powerOutput" type="number" placeholder="Power Output (kW)" required
              class="w-full border border-gray-300 rounded-md px-3 py-2" />
